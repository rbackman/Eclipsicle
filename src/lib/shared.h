...
static const std::vector<IntParameter> getDefaultIntParameters()
{
    ...
    return {
        ...
        {PARAM_CURRENT_LED, "CurrentLED", 1, 0, 500},
...
static const std::vector<FloatParameter> getDefaultFloatParameters()
{
    ...
    return {
        ...
        {PARAM_REPEAT, "Repeat", 0.1, 0.1, 10.0},
...
struct Vec3D
{
    ...
    Vec3D(const Node3D &node) : x(node.x), y(node.y), z(node.z) {}
};
struct Segment
{
    std::string name;
    ...
    Segment() = default;
    Segment(const std::string &name, const std::vector<Vec3D> &points) : name(name), points(points) {}
};
struct StructureDef
{
    //  all the points in a structure
    std::string name;

    std::vector<Segment> node_segments;
};
